...
def plot_both_side_muda(
    ...
    **kwargs :
    ...
    ax[0].axhline(right_price, linestyle='--', c='k', label='Trading price determined by right players')
    ax[0].legend()
    ax[0].set_title('Left players')

    plot_demand_curves(bids[bids.index.isin(right_players)], ax=ax[1])
    ax[1].axhline(left_price, linestyle='--', c='k', label='Trading price determined by left players')
    ax[1].legend()
